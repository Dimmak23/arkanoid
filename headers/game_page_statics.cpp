...
Game::StaticUnits::StaticUnits(const Utilities::StaticUnits& utils)
{
	...
	initialize_frame(legend_frame, to_f(legend_field.overall_width), to_f(legend_field.overall_height), sf::Color::Red,
					 sf::Color::Black, to_f(outline.outside_thk), to_f(legend_field.origin_x),
					 to_f(legend_field.origin_y));

	//===MAIN_FIELD_INIT===

	initialize_frame(game_frame, to_f(game_field.overall_width), to_f(game_field.overall_height), sf::Color::Red,
					 sf::Color::Black, to_f(outline.outside_thk), to_f(game_field.origin_x), to_f(game_field.origin_y));

	//==SCORE_FIELD_INIT==

	initialize_frame(status_frame, to_f(status_field.overall_width), to_f(status_field.overall_height), sf::Color::Red,
					 sf::Color::Black, to_f(outline.outside_thk), to_f(status_field.origin_x),
	...
	for (auto& line : extension_lines)
	{
		...
		line.setPosition(
			sf::Vector2f(to_f(game_field.origin_x), to_f(game_field.origin_y + Default::conveyor_mapping.at(index++))));
	}
	...
	for (auto& circle : lifes_outline)
	{
		...
		circle.setPosition(sf::Vector2f(status_labels.at(LIFES_LABEL).getGlobalBounds().left + 10.f + hor_adder,
										status_labels.at(LIFES_LABEL).getGlobalBounds().top +
											status_labels.at(LIFES_LABEL).getGlobalBounds().height + 40.f +
											vert_adder));
	...
	for (auto& parameter : ball_parameters)
	{
		...
		if (index < 3)
			ver_adder = 30.f * index;
		else
			ver_adder = 30.f * (index - 3);

		if (!(index % 3)) hor_adder = to_f(status_field.overall_width / 2 - 10.f);
	}
	...
	{
		initialize_frame(frame, 90.f, 25.f, sf::Color::Red, sf::Color::Black, 0.75f,
	...
	*/
	...
	{
		while (std::ranges::find(skipped_abl_textures_addresses, texture_address) !=
			   skipped_abl_textures_addresses.end())
		{
			texture_address++;
		}
		Utilities::safe_parse(
	...
	{
		while (std::ranges::find(skipped_pdl_textures_addresses, texture_address) !=
			   skipped_pdl_textures_addresses.end())
		{
			texture_address++;
		}
		Utilities::safe_parse(
	...
	for (auto& sprite : blk_sprites)
	{
		...
		sprite.setPosition(sf::Vector2f(blocks_label.getGlobalBounds().left,
	...
	for (auto& sprite : abl_sprites)
	{
		...
		sprite.setPosition(sf::Vector2f(abilities_label.getGlobalBounds().left,
