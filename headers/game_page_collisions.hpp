...
#define BLOCK_BOTTOM                                                                                                   \
	dynamics.conveyor.at(indexer).getGlobalBounds().top + dynamics.conveyor.at(indexer).getGlobalBounds().height
#define BLOCK_LEFT dynamics.conveyor.at(indexer).getGlobalBounds().left
#define BLOCK_RIGHT                                                                                                    \
	dynamics.conveyor.at(indexer).getGlobalBounds().left + dynamics.conveyor.at(indexer).getGlobalBounds().width

#define ABILITY_CENTER                                                                                                 \
	dynamics.conveyor.at(indexer).getGlobalBounds().left + dynamics.conveyor.at(indexer).getGlobalBounds().width / 2
#define ABILITY_TOP dynamics.conveyor.at(indexer).getGlobalBounds().top
#define ABILITY_BOTTOM                                                                                                 \
	dynamics.conveyor.at(indexer).getGlobalBounds().top + dynamics.conveyor.at(indexer).getGlobalBounds().height
...
#define THIS_ABL_L_SIDE                                                                                                \
	(THIS_ABL_Y_CENTER < ABLOCK_BOTTOM) && (THIS_ABL_Y_CENTER > ABLOCK_TOP) && (THIS_ABL_RIGHT > ABLOCK_LEFT) &&       \
		(THIS_ABL_X_CENTER < ABLOCK_LEFT)
#define THIS_ABL_R_SIDE                                                                                                \
	(THIS_ABL_Y_CENTER < ABLOCK_BOTTOM) && (THIS_ABL_Y_CENTER > ABLOCK_TOP) && (THIS_ABL_LEFT < ABLOCK_RIGHT) &&       \
		(THIS_ABL_X_CENTER > ABLOCK_RIGHT)
#define THIS_ABL_T_SIDE                                                                                                \
	(THIS_ABL_X_CENTER < ABLOCK_RIGHT) && (THIS_ABL_X_CENTER > ABLOCK_LEFT) && (THIS_ABL_BOTTOM > ABLOCK_TOP)
#define THIS_ABL_B_SIDE                                                                                                \
	(THIS_ABL_X_CENTER < ABLOCK_RIGHT) && (THIS_ABL_X_CENTER > ABLOCK_LEFT) && (THIS_ABL_TOP < ABLOCK_BOTTOM)
...
#define PADDLE_ROX                                                                                                     \
	dynamics.paddle->getGlobalBounds().left + dynamics.paddle->getGlobalBounds().width - to_f(Default::block_height / 2)
...
#define FIRST_BLOCK_BOTTOM                                                                                             \
	(dynamics.conveyor).at(0).getGlobalBounds().top + (dynamics.conveyor).at(0).getGlobalBounds().height
...
inline static bool hasValue(sf::Rect<Type>& rectangle)
{
	auto size = rectangle.getSize();
	auto hypotenous = sqrt(size.x * size.x + size.y * size.y);
	return hypotenous > 0;
}
...
inline static colission_detection checkCornerCollision(const float& corner_X, const float& corner_Y,
													   const float& centerX_BLL, const float& centerY_BLL,
													   const float& rad_BLL, const float& dy_BLL)
{
	...
	// If coming from UP (dy_BLL>0)
		...
		move_y = -(rad_BLL) * (std::tan(angle));

	/// GLITCH (BALL GOES TO FAR FROM PADDLE) SOLUTION
	if (move_y > rad_BLL)
		move_y = rad_BLL;
	else if (move_y < -rad_BLL)
		move_y = -rad_BLL;
...
inline static colission_detection checkCircleCollision(const float& centerX_PDL, const float& centerY_PDL,
													   const float& rad_PDL, const float& centerX_BLL,
													   const float& centerY_BLL, const float& rad_BLL,
													   const float& dy_BLL = 0.f)
{
	...
	// If coming from UP (dy_BLL>0)
		...
		move_y = -(rad_PDL + rad_BLL - hypotenuse) * (std::tan(angle));
...
inline static void collideUnits(Game::dynamicUnits& dynamics, const float& d_time)
{
	...
	if ((PADDLE_LEFT < GFIELD_LEFT + 10.f)			   // 10.f pixels near the border is a SLOWDOWN ZONE
	...
	if ((PADDLE_RIGHT > (GFIELD_RIGHT - 10.f)) &&
	...
	{
		dynamics.ball->setPosition(sf::Vector2f(GFIELD_LEFT + BALL_WIDTH / 2, dynamics.ball->getPosition().y));
	...
	{
		dynamics.ball->setPosition(sf::Vector2f(GFIELD_RIGHT - BALL_WIDTH / 2, dynamics.ball->getPosition().y));
	...
	{
		dynamics.ball->setPosition(sf::Vector2f(dynamics.ball->getPosition().x, GFIELD_TOP + BALL_HEIGHT / 2));
		...
		(!Game::dynamicUnits::lost_ball))
	{
		...
		auto [collision_detection, vector] =
			checkCircleCollision(PADDLE_LOX, PADDLE_LOY, PADDLE_RAD, BALL_OX, BALL_OY, BALL_RAD);
		...
		(!Game::dynamicUnits::lost_ball))
	{
		...
		// And, MOST IMPORTANTLY: Do the circle of the BALL go thru PADDLE circle?
		auto [collision_detection, vector] =
			checkCircleCollision(PADDLE_ROX, PADDLE_ROY, PADDLE_RAD, BALL_OX, BALL_OY, BALL_RAD);
	...
	for (int indexer{}; indexer < dynamics.conveyor.size();)
	{
		...
		diagnose.getSize();
		ABL_BODY.intersects(PADDLE_BODY, diagnose);

		if (hasValue(diagnose))
		{
...
static inline void ablHitPlayGround(Game::dynamicUnits& dynamics, const float& d_time)
{
	...
	auto out_of_Lwall = [&](const sf::Sprite& test) { return (test.getGlobalBounds().left < GFIELD_LEFT); };
	this_abl = std::find_if(dynamics.conveyor.begin(), dynamics.conveyor.end(), out_of_Lwall);
	...
	this_abl = std::find_if(dynamics.conveyor.begin(), dynamics.conveyor.end(), out_of_Rwall);
	...
	auto out_of_Ceil = [&](const sf::Sprite& test) { return (test.getGlobalBounds().top < GFIELD_TOP); };
	this_abl = std::find_if(dynamics.conveyor.begin(), dynamics.conveyor.end(), out_of_Ceil);
	...
	this_abl = std::find_if(dynamics.conveyor.begin(), dynamics.conveyor.end(), out_of_field);
...
inline static void ballHitBlocks(Game::dynamicUnits& dynamics, const float& d_time)
{
	...
	if (BALL_TOP > FIRST_BLOCK_BOTTOM) return;
	...
	auto only_blocks = [](const Game::block_map_variables& struc) { return struc.block; };

	// If there is gonna be a collision?
	auto bounds_collision = [&](const sf::Sprite& unit) {
		return ((U_HOR_TOUCH(BALL_LEFT) && (U_VER_TOUCH(BALL_TOP) || U_VER_TOUCH(BALL_BOTTOM))) ||
	...
	// Find next block
	give_block = std::find_if(give_block, dynamics.conveyor_map.end(), only_blocks);
	...
	while (give_block != dynamics.conveyor_map.end())
	{
		...
		attacked_unit = std::find_if(attacked_unit, dynamics.conveyor.end(), bounds_collision);
		...
		give_block = std::find_if(give_block + 1, dynamics.conveyor_map.end(), only_blocks);
	}

	// We didn't reach the end of the conveyor
	// We will collide with ONLY ONE BLOCK
	if ((attacked_unit != dynamics.conveyor.end()) && (give_block != dynamics.conveyor_map.end()) &&
		((*give_block).block))
	{
		///<---ASERTION
		...
		// If it is a corner
		if ((BALL_UL_CORNER || BALL_BL_CORNER || BALL_UR_CORNER || BALL_BR_CORNER))
		{
...
inline static void ballHitAbilities(Game::dynamicUnits& dynamics, const float& d_time)
{
	...
	// Find next ability
	give_ability = std::find_if_not(give_ability, dynamics.conveyor_map.end(), only_blocks);
	...
	while (give_ability != dynamics.conveyor_map.end())
	{
		...
		// If not: find next block
		give_ability = std::find_if_not(give_ability + 1, dynamics.conveyor_map.end(), only_blocks);
	}

	// We didn't reach the end of the conveyor
	// We will collide with ONLY ONE ABILITY
	if ((attacked_unit != dynamics.conveyor.end()) && (give_ability != dynamics.conveyor_map.end()) &&
		!((*give_ability).block))
	{
		///<---ASERTION
		// Make iterators points to the same positions
		...
		if ((BALL_OX < AABL_RX_LEFT) || (BALL_OX > AABL_RX_RIGHT))
		{
			...
			{
				abl_result =
					checkCircleCollision(AABL_RX_RIGHT, AABL_Y_CENTER, AABL_RAD, BALL_OX, BALL_OY, BALL_RAD, BALL_DY);
			}
			// From the LEFT side of the ABILITY
			else
			{
				abl_result =
					checkCircleCollision(AABL_RX_LEFT, AABL_Y_CENTER, AABL_RAD, BALL_OX, BALL_OY, BALL_RAD, BALL_DY);
			}
...
#define TU_COLLIDE_T_ABL                                                                                               \
	((TU_RX_LEFT > AU_RX_LEFT) && (TU_RX_LEFT < AU_RX_RIGHT) ||                                                        \
	 (TU_RX_RIGHT > AU_RX_LEFT) && (TU_RX_RIGHT < AU_RX_RIGHT)) &&                                                     \
		(TU_BOTTOM > AU_TOP) && (TU_TOP < AU_TOP)
#define TU_COLLIDE_B_ABL                                                                                               \
	((TU_RX_LEFT > AU_RX_LEFT) && (TU_RX_LEFT < AU_RX_RIGHT) ||                                                        \
	 (TU_RX_RIGHT > AU_RX_LEFT) && (TU_RX_RIGHT < AU_RX_RIGHT)) &&                                                     \
		(TU_TOP < AU_BOTTOM) && (TU_BOTTOM > AU_BOTTOM)
#define TU_COLLIDE_L_ABL                                                                                               \
	(TU_RIGHT > AU_LEFT) && (TU_RX_RIGHT < AU_RX_LEFT) &&                                                              \
		((TU_TOP < AU_BOTTOM) && (TU_TOP > AU_TOP) || (TU_BOTTOM < AU_BOTTOM) && (TU_BOTTOM > AU_TOP))
#define TU_COLLIDE_R_ABL                                                                                               \
	(TU_LEFT < AU_RIGHT) && (TU_RX_LEFT > AU_RX_RIGHT) &&                                                              \
		((TU_TOP < AU_BOTTOM) && (TU_TOP > AU_TOP) || (TU_BOTTOM < AU_BOTTOM) && (TU_BOTTOM > AU_TOP))

#define TU_COLLIDE_T_BLK                                                                                               \
	((TU_RX_LEFT > AU_LEFT) && (TU_RX_LEFT < AU_RIGHT) || (TU_RX_RIGHT > AU_LEFT) && (TU_RX_RIGHT < AU_RIGHT)) &&      \
		(TU_TOP < AU_TOP) && (TU_BOTTOM > AU_TOP)
#define TU_COLLIDE_B_BLK                                                                                               \
	((TU_RX_LEFT > AU_LEFT) && (TU_RX_LEFT < AU_RIGHT) || (TU_RX_RIGHT > AU_LEFT) && (TU_RX_RIGHT < AU_RIGHT)) &&      \
		(TU_BOTTOM > AU_BOTTOM) && (TU_TOP < AU_BOTTOM)
#define TU_COLLIDE_L_BLK                                                                                               \
	(TU_Y_CENTER > AU_BOTTOM) && (TU_Y_CENTER < AU_TOP) && (TU_RIGHT > AU_LEFT) && (TU_LEFT < AU_LEFT)
#define TU_COLLIDE_R_BLK                                                                                               \
	(TU_Y_CENTER > AU_BOTTOM) && (TU_Y_CENTER < AU_TOP) && (TU_LEFT < AU_RIGHT) && (TU_RIGHT > AU_RIGHT)
#define TU_COLLIDE_UL_BLK (TU_BOTTOM > AU_TOP) && (TU_Y_CENTER < AU_TOP) && (TU_RIGHT > AU_LEFT) && (TU_LEFT < AU_LEFT)
#define TU_COLLIDE_BL_BLK                                                                                              \
	(TU_TOP < AU_BOTTOM) && (TU_Y_CENTER > AU_BOTTOM) && (TU_RIGHT > AU_LEFT) && (TU_LEFT < AU_LEFT)
#define TU_COLLIDE_UR_BLK                                                                                              \
	(TU_BOTTOM > AU_TOP) && (TU_Y_CENTER < AU_TOP) && (TU_LEFT < AU_RIGHT) && (TU_RIGHT > AU_RIGHT)
#define TU_COLLIDE_BR_BLK                                                                                              \
	(TU_TOP < AU_BOTTOM) && (TU_Y_CENTER > AU_BOTTOM) && (TU_LEFT < AU_RIGHT) && (TU_RIGHT > AU_RIGHT)
...
inline static void resolveCollision(sf::Sprite& this_unit, sf::Sprite& attacked_unit,
									Game::block_map_variables& this_map, Game::block_map_variables& attacked_map,
									const int& subCategory)
{
	if (subCategory == UNDEFINED_TYPE)
	{
		// std::cout << "Undefined type of collision\n";
		// std::cout << "this_unit: {" << TU_LEFT - game_field.origin_x << ", " << TU_TOP - game_field.origin_y << "}.
		// map.operand: " << this_map.operand << " \n"; std::cout << "attacked_unit: {" << AU_LEFT - game_field.origin_x
		// << ", " << AU_TOP - game_field.origin_y << "}. map.operand: " << attacked_map.operand << " \n";
	}
	...
	{
		sharp_collision = checkCircleCollision(AU_RX_LEFT, AU_Y_CENTER, AU_RAD, TU_RX_RIGHT, TU_Y_CENTER, TU_RAD);
	}
	else if (subCategory == ABL_AT_RIGHT_ABL)
	{
		sharp_collision = checkCircleCollision(AU_RX_RIGHT, AU_Y_CENTER, AU_RAD, TU_RX_LEFT, TU_Y_CENTER, TU_RAD);
	}
	else if (subCategory == ABL_AT_UL_BLK)
	{
		sharp_collision = checkCornerCollision(AU_LEFT, AU_TOP, TU_RX_RIGHT, TU_Y_CENTER, TU_RAD, THIS_MAP_DY);
	}
	else if (subCategory == ABL_AT_BL_BLK)
	{
		sharp_collision = checkCornerCollision(AU_LEFT, AU_BOTTOM, TU_RX_RIGHT, TU_Y_CENTER, TU_RAD, THIS_MAP_DY);
	}
	else if (subCategory == ABL_AT_UR_BLK)
	{
		sharp_collision = checkCornerCollision(AU_RIGHT, AU_TOP, TU_RX_LEFT, TU_Y_CENTER, TU_RAD, THIS_MAP_DY);
	}
	else if (subCategory == ABL_AT_BR_BLK)
	{
		sharp_collision = checkCornerCollision(AU_RIGHT, AU_BOTTOM, TU_RX_LEFT, TU_Y_CENTER, TU_RAD, THIS_MAP_DY);
	}
...
inline static int getTypeCollision(sf::Sprite& this_unit, sf::Sprite& attacked_unit,
								   Game::block_map_variables& this_map, Game::block_map_variables& attacked_map,
								   const int& category)
{
...
inline static bool checkBoundCollision(const sf::Sprite& this_unit, const sf::Sprite& attacked_unit)
{
	...
	sf::Rect<float> diagnose;
	THIS_U.intersects(ATTACK_U, diagnose);

	return hasValue(diagnose);
}

// Belt conveqyor iterator, for ability->ability and ability->block collisions
inline static void iterate2Dcontainers(std::vector<sf::Sprite>& t_unit, std::vector<Game::block_map_variables>& t_map,
									   // TRUE: we desire to test ability/block collisions, FALSE: we desire to test
									   // ability/ability collisions
									   const bool& desire)
{
		...
		for (int holder_index{}; holder_index < dynamics.conveyor.size(); holder_index++)
		{
			...
			if (
				...
				(HOLDER_TOP == btm_coor) && (
												...
												//! WE ARE ALREADY PUSH ABILITIES WHEN THY ARE COLLIDE BLOCKS AND OTHER
												//! ABILITIES!
