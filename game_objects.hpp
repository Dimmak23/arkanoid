...
class GameObjects
{
	public:
		...
		GameObjects(const int& blocks_qty) : blocks_quantity(blocks_qty)
		{
			...
			//Initialize ball
			balls_texture.resize(BALL_COUNT);
			dummy_parser = 61 ;
			//Initialize textures for the ball
			for (auto& ball: balls_texture)
			{
				parse_texture(ball, dummy_parser++);
			}

			//Initialize small texture for ball sprite
			//set_texture(ball, balls_texture, SMALL);

			
			//==========PADDLE==========

			//Initialize paddle
			paddles_texture.resize(PADDLE_COUNT);
			dummy_parser = 50;
			//Initialize textures for the paddle
			for (auto& paddle : paddles_texture)
			{
				parse_texture(paddle, dummy_parser++);
			}

			//Initialize capsule texture for paddle sprite
			//set_texture(paddle, paddles_texture, CAPSULE_1);
		...
		void set_texture(parameters& item, const std::vector<sf::Texture>& donor, const int& texture) noexcept;
	...
	public:
		...
		//=___=PADDLE=___=
		parameters paddle;
		std::vector<sf::Texture> paddles_texture;

		//=___=BALL=___=
		parameters ball;
		std::vector<sf::Texture> balls_texture;
...
inline void GameObjects::parse_texture(sf::Texture& item, const int& number_in_dir) noexcept
{
	...
	//if(number_in_dir == 56) parsing_status = item.loadFromFile(path, sf::IntRect(sf::Vector2i(0, 0), sf::Vector2i(693, 128)));
	//else if(number_in_dir == 57) parsing_status = item.loadFromFile(path, sf::IntRect(sf::Vector2i(0, 0), sf::Vector2i(230, 128)));
	//else
...
inline void GameObjects::set_texture(parameters& item, const std::vector<sf::Texture>& donor, const int& texture) noexcept
{
	item.sprite.setTexture(donor.at(texture), true); // resetRect property: change size of the sprite to suit the next texture
}
