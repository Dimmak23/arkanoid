...
int main()
{
	...
	sf::RenderWindow application_window(sf::VideoMode(screen_s.x, screen_s.y, desktop.bitsPerPixel),
		...
		while (Util::Process::running)
		{
			...
			while (Game::Process::running)
			{
				...
				if (!Game::Process::on_pause)
				{
					...
					fork_Game_DUnits->updateParInterface(Game::dynamicUnits::ball_kinematics,
														 fork_Game_DUnits->ball_parameters);

					// Update paddle parameters interface
					fork_Game_DUnits->updateParInterface(Game::dynamicUnits::paddle_kinematics,
														 fork_Game_DUnits->paddle_parameters);
			...
			std::unique_ptr<Outro::staticUnits> fork_Outro_SUnits =
				std::make_unique<Outro::staticUnits>(*fork_Util_SUnits);
